...
- **Device Pool & Provisioning (enterprise operations):** extended `spanda-config` with `Active` lifecycle state, health/calibration readiness gates (`device_health`), quarantine policy (`device_quarantine`), pool operations (`assign`/`unassign`/`quarantine`/`retire`), identity anomaly detection, multi-transport discovery registry (subnet, mDNS, BLE, USB, CAN, MQTT, ROS2 stubs), device report bundle, config persistence (`device_config_persist`), failover chains (`device_failover`) integrated with recovery via `enrich_recovery_plan_with_failover`; CLI `spanda device provision|assign|unassign|quarantine|retire` with `--write`; Control Center API `GET /v1/devices/{id}`, `POST /v1/devices/discover`, per-device provision/assign/quarantine, `GET /v1/robots|fleets|device-tree|failover/chains|device-reports`, `POST /v1/readiness/run`; expanded `@spanda/web` Control Center panel with provision/assign/quarantine actions; registry package `spanda-discovery-mdns`; docs [device-pool.md](docs/device-pool.md), [device-provisioning.md](docs/device-provisioning.md), [device-discovery.md](docs/device-discovery.md), [device-quarantine.md](docs/device-quarantine.md), [calibration.md](docs/calibration.md). OTLP/JSON trace export to Jaeger (`GET /v1/observability/otlp/traces`, `POST /v1/observability/otlp/export`, `SPANDA_OTLP_TRACES_ENDPOINT`, `SPANDA_OTLP_TRACE_AUTO_PUSH`); WebSocket live telemetry stream (`WS /v1/stream/telemetry`); Python `TelemetryStream` (`pip install spanda-sdk[stream]`); smoke probes `scripts/mock_otlp_traces_collector.py`, `scripts/ws_telemetry_probe.py`.
...
- **Docs:** enterprise operations doc-sync — root README, getting-started, roadmap E4 exit criteria, feature-status tiers, enterprise-operations pillar matrix, product-strategy Control Center status.
