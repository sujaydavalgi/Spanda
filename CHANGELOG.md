...
- **AI-assisted development (mock-first):** new `spanda-generate` crate with template scaffolds and `suggest_program`; CLI `spanda generate mission|robot|health-policy` and `spanda suggest <file.sd>`; `scripts/generate_smoke.sh`; docs [ai-assisted-development.md](docs/ai-assisted-development.md).
- **Spoofing detection:** new `spanda-spoofing` crate with program coverage and trace plausibility analysis; CLI `spanda spoof-check <file.sd|file.trace> [--json]`; showcase `examples/showcase/gps_spoofing/`; `scripts/spoof_smoke.sh`; docs [spoofing-detection.md](docs/spoofing-detection.md).
