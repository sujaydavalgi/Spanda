...
- **Trust showcase demos:** `examples/showcase/package_tampering/`, `mission_tampering/`, `runtime_intrusion/` with tamper and integrity workflows; `spanda demo trust` one-command walkthrough; trust showcases and minimal trust registry bundled in CLI crate via `scripts/sync_bundled_examples.sh` and `scripts/sync_bundled_registry.sh`; `scripts/bundled_trust_smoke.sh`.
