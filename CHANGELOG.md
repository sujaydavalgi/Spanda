...
- **Secure-boot contract integration:** `spanda-tamper::secure_boot` scores `trust.jetson` / `trust.pi` in `tamper-check` and `integrity`; `examples/showcase/secure_boot/`; `scripts/secure_boot_smoke.sh`.
- **Composite trust in gates and scorecard:** `spanda deploy gate` adds `composite_trust` gate; scorecard security pillar blends threat risk and composite trust; `spanda explain` includes `composite_trust` section.
