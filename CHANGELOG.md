...
- **Persistent telemetry store:** HTTP scrape server (`spanda telemetry serve`); OTLP/JSON export; per-event `session_id` tagging; Prometheus export; SQLite backend (`SPANDA_TELEMETRY_BACKEND=sqlite`) with automatic JSONL migration on first open; `spanda telemetry sessions` and `spanda telemetry replay --session`
