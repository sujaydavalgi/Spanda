...
- **Persistent telemetry store:** `spanda-telemetry-store` crate — append-only `.spanda/telemetry-store.jsonl` for device/sensor events; `.spanda/telemetry-heartbeats.json` for task liveness; `--persist-telemetry` / `SPANDA_TELEMETRY_STORE=1`; CLI `spanda telemetry list|latest|heartbeats|stats|export`
