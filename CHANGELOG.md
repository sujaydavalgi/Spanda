...
- **Secure-boot contract integration:** `spanda-tamper::secure_boot` scores `trust.jetson` / `trust.pi` in `tamper-check` and `integrity`; optional `SPANDA_ATTESTATION_ENDPOINT` live attestation; deploy agent `/v1/status` attestation fields; `examples/showcase/secure_boot/`; `scripts/secure_boot_smoke.sh`; docs [hardware-attestation.md](docs/hardware-attestation.md).
