...
use spanda_interpreter::simulator::{create_default_simulator, Simulator, SimulatorConfig};
...
impl DebugMachine {
    ...
    pub fn run_until_pause(&mut self, step: DebugStepKind) -> Result<DebugSession, SpandaError> {
        ...
        if step == DebugStepKind::Continue {
            self.controller
                .command(spanda_debug::DebugCommand::Continue);
            ...
            self.controller.command(spanda_debug::DebugCommand::Step);
        } else {
            self.step_out_target_depth = None;
            self.controller.command(spanda_debug::DebugCommand::Step);
        }
