...
use crate::fleet_orchestrator::PeerDelivery;
use crate::fleet_remote::{
    default_fleet_agents_path, load_fleet_agent_registry, relay_peer_deliveries,
    FleetAgentRegistry,
};
...
pub fn relay_deliveries_via_mesh(
    ...
    let parsed = crate::deploy_http::parse_http_url(mesh_url)?;
    let url = format!(
        "{}://{}:{}/v1/mesh/relay",
    ...
    if response.status >= 400 {
        return Err(format!("fleet mesh HTTP {}: {}", response.status, response.body));
    }
