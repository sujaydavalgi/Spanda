...
| Step | Status |
|------|--------|
| Route runtime imports through workspace crates (`spanda-ast`, `spanda-runtime`, …) instead of `crate::` | **In progress** — kernel modules + `spanda-comm` + **`spanda-safety`** + **`spanda-hal`** + **`spanda-transport-routing`**; **remaining:** `ai`, `providers` bootstrap, `concurrency`, `error` |
